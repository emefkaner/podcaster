...
await p.goto('http://localhost:3033/login', { waitUntil:'domcontentloaded' });
...
const r = await p.evaluate(async () => {
  ...
  try {
    ...
    const folge=await mod.lokalZusammenbauen([t1,t2],
      {introUrl:s.introUrl,outroUrl:s.outroUrl},()=>{});
    return { ok:true, introUrl:s.introUrl, outroUrl:s.outroUrl,
             teile:[t1.size,t2.size], folge:folge.size };
  } catch(e){ return { ok:false, introUrl:s.introUrl, fehler:e.message }; }
});
console.log('\n=== DURCHLAUF ===');
